...
5. modern 审批触发通过 `item/commandExecution/requestApproval` 事件映射；如事件未及时可见，则以 `thread.status.activeFlags=waitingOnApproval` 兜底识别。
6. modern 审批恢复优先使用 server-request 原生响应（按 request id 回写 `accept/cancel`），仅在缺失 modern token 时回退 legacy `turn.resume`。
7. codex backend 不可用或行为不稳定时，可通过环境变量快速回退到 `mock`。
8. 中断仅对活动态 operation 生效；已终态 operation 保持原状态，避免竞态改写。
